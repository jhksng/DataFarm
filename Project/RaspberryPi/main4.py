...
def capture_and_upload():
    ...
    with camera_lock:
        try:
            ...
            directory_path = "datafarm/picture" # ✨ 수정: 정확한 폴더 경로 정의
            os.makedirs(directory_path, exist_ok=True)
            next_id = get_next_id()
            
            # ✨ 수정: 요청하신 파일 이름 형식 (farm01_cam01_401_20251012_091032.jpg와 유사하게)
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            file_name = f"{FARM_ID}_{CAMERA_ID}_{next_id}_{timestamp}.jpg" 
            ...
            # 1. 서버에 사진 등록 알림 (Spring의 savePhoto 호출)
            # 💡 NOTE: 이 토픽은 서버의 'savePhoto'를 호출하는 토픽이어야 합니다.
            # 서버가 MQTT_CONTROL_TOPIC에서 "capture"를 받고, 이전에 등록을 위한 별도 토픽이 있을 수 있습니다.
            # 여기서는 파일 업로드 완료 후 등록을 위한 토픽이 있다고 가정하고 임시로 주석 처리합니다.
            # client.publish("photo/upload/complete", file_name)  

            # 2. ✨ NEW: 이미지 분석 및 MQTT 발행
