...
def capture_and_upload(source):
    ...
    with camera_lock:
        try:
            ...
            # --- 변경된 부분 ---
            # 새로운 폴더 구조: raspi-01/id/source/timestamp.jpg
            # GCS는 '/'를 폴더 구분자로 인식합니다.
            gcs_path = f"{DEVICE_ID}/{next_id}/{source}/{time.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
            # 로컬에 저장할 파일 경로는 플랫하게 유지합니다.
            local_file_name = os.path.basename(gcs_path)
            file_path = os.path.join(directory_path, local_file_name)
            # -----------------
            ...
            # GCS에 업로드할 Blob 이름은 전체 경로인 gcs_path를 사용합니다.
...
def on_message(client, userdata, msg):
    ...
    if msg.topic == MQTT_CONTROL_TOPIC and msg.payload.decode() == "capture":
        ...
        capture_and_upload("user")  # 'user' 폴더에 저장
        return
...
if __name__ == '__main__':
    ...
    try:
        ...
        while True:
            ...
            if current_time - last_auto_capture_time >= auto_capture_interval:
                print("Executing scheduled photo capture command...")
                capture_and_upload("auto")  # 'auto' 폴더에 저장
