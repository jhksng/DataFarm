...
MQTT_DATA_TOPIC = "datafarm/sensor_data"
# TODO: 서버로부터 제어 명령을 받을 토픽 이름으로 변경하세요.
MQTT_CONTROL_TOPIC = "datafarm/module_state"
# 클라이언트 ID는 퍼블리셔/서브스크라이버 통일
MQTT_CLIENT_ID = "raspi-client-01"
...
def on_message(client, userdata, msg):
    """서버로부터 메시지를 받았을 때 호출되는 콜백 함수"""
    try:
        # 받은 메시지 페이로드를 JSON으로 디코딩합니다.
        control_data = json.loads(msg.payload.decode('utf-8'))
        print("\n=====================================================")
        print(f"  Received control command from server:")
        for module, command in control_data.items():
            print(f"    - Module: {module}, Command: {command}")
            # 여기에 실제 하드웨어를 제어하는 로직을 추가할 수 있습니다.
            # 예: GPIO.output(17, GPIO.LOW)
        print("=====================================================\n")
...
def publish_data(client, soil_p, water_p, temp_avg, humi_avg):
    ...
    try:
        data = {
            ...
            "humi": round(humi_avg, 2),
            "light": 100, # 임의의 조도 센서 값
            "crop": "상추", # 임의의 작물 이름
        ...
        result = client.publish(MQTT_DATA_TOPIC, json_data, qos=1)
